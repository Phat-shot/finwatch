...
- The Music category could come up empty ("No libraries found") even when
  ...
  without trouble: artists were requested through `/Items`, which does not
  return them reliably. The app now uses the dedicated artists endpoint —
  the same one the web UI's "Artists" tab uses.
